...
SELECT
    ...
    TIMESTAMP_SUB(
        CURRENT_TIMESTAMP(), INTERVAL CAST(RAND() * 365 AS INT64) DAY
    ) AS fecha,
    CAST(ROUND(RAND() * 5000, 2) AS NUMERIC) AS monto,
    ['DEPOSITO', 'RETIRO', 'TRANSFERENCIA', 'PAGO_TARJETA'][
        OFFSET(CAST(FLOOR(RAND() * 4) AS INT64))
    ] AS tipo,
    ['APP', 'WEB', 'SUCURSAL', 'ATM'][
        OFFSET(CAST(FLOOR(RAND() * 4) AS INT64))
    ] AS canal,
